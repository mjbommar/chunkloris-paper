...
exponent $\sim 1.7$--$2.0$ in our $N$ range) via a
growing-retained-buffer re-scan in the \fname{Readable} substrate
(not, as one might assume, microtask-queue growth). A
...
The widely-advised "deploy behind nginx" mitigation is
empirically confirmed: with default
\fname{proxy\_request\_buffering~on}, an upstream-instrumented
end-to-end probe shows the upstream receiving a single
\fname{Content-Length}-framed request in one \fname{recv()}, the
chunk stream fully coalesced. \fwid{HAProxy} does \emph{not}
aggregate (it streams by default, and \fname{http-buffer-request}
waits only for one buffer), so direct-exposed, HAProxy-fronted, or
buffering-disabled deployments inherit the full per-chunk cost.
