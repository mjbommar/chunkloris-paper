...
cost (Section~\ref{sec:synth-matrix} shows nginx-as-origin
costs $\sim$\SI{114}{\micro\second}/chunk -- the worst linear cell
in our sample) but nginx is a hardened C server with well-tuned
worker concurrency, so the impact is bounded.
