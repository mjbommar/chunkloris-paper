...
\subsection{Node.js (the three QUADRATIC servers)}
\label{sec:pf-node}
...
respectively\footnote{Both the \fwid{express} and \fwid{fastify}
$N{=}\num{250000}$ cells are extrapolated from a quadratic fit and
are marked with an asterisk in Table~\ref{tab:matrix}; only the
underlying $N{=}\num{50000}$ and $N{=}\num{100000}$ data points
were measured directly. The directly-measured \SI{1.5}{\mebi\byte}
headline figure we cite (282\,s of single-core wall time for Node
\fwid{http.Server}) is a measurement at $N{=}\num{250000}$; the
\fwid{express}/\fwid{fastify} 324/339\,s figures are not, and are
excluded from any measured total.}. The
...
The quadratic mechanism is the \fname{maybeReadMore\_}
\fname{read(0)} loop re-scanning the \fname{Readable}'s retained
internal buffer (\fname{\_readableState.buffer}, a plain array)
once per event-loop turn while the buffer drains slower than it
fills; the per-turn cost and V8 scavenge cost both grow with the
retained buffer size, which grows with chunk count, giving
$\Theta(N^2)$. (The \fname{maybeReadMore} \fname{nextTick} is
re-armed under a \fname{kReadingMore} guard and is \emph{not}
itself quadratic.)
...
\fwid{Bandit} 1.11.1 (pure-Elixir HTTP/1, no Cowboy) is a
high-constant \emph{linear} case ($\sim$\SI{138}{\micro\second}/chunk
under Mode B), \emph{not} quadratic: its measured scaling
exponent is $\sim 1.05$ and its per-chunk cost \emph{falls} with
$N$ (155\,$\to$\,148\,$\to$\,138\,\si{\micro\second} over
$N{=}$50k\,$\to$\,100k\,$\to$\,250k). It nonetheless carries the
same "per-chunk operation walks accumulated state" shape as the
Node trio: \fname{erlang:iolist\_size/1} consumes 89.3\% of CPU
in the \fname{:eprof} sample, because
\fname{Bandit.HTTP1.Socket.do\_read\_chunk!/4} accumulates body
fragments into an iolist and re-walks it on every chunk via
\fname{iolist\_size/1} to update progress. In our $N$ range that
walk stays bounded enough to remain linear; we classify Bandit
VULNERABLE-PER-CHUNK at the top of that class.

\subsection{.NET (the HTTP/1.1 BATCHES-CORRECTLY reference)}
\label{sec:pf-dotnet}

\fwid{Kestrel} 9 (ASP.NET Core 9) on .NET 9 is the single
HTTP/1.1 server in our sample that defeats the per-chunk class
structurally (four WebSocket servers do the same at the frame
layer; Section~\ref{sec:pf-ws}). \fname{Http1ChunkedEncodingMessageBody.PumpAsync}
...
\SI{114}{\micro\second}/chunk -- among the worst per-chunk cells in
our sample (second only to the \fwid{bandit} iolist outlier at
$\sim$\SI{138}{\micro\second}), and a synthetic figure rather than
a representative nginx deployment: the C cost is amplified by the
OpenResty Lua-sink wrapper that we used to obtain a sink-handler in
nginx's request lifecycle. nginx's real-world role is as the
buffering reverse proxy of Section~\ref{sec:mitigation}, not as an
origin app server. Apache \fwid{httpd} 2.4.67 (\fname{event} MPM,
...
All six exhibit the same per-frame amplification at the DATA-frame
level (recorded as \textsc{VULNERABLE-PER-CHUNK} in the dataset's
verdict taxonomy, which does not distinguish the h2 frame case),
